...
Function xBulanIndo3($xBulanIndo)
{
	if($xBulanIndo == 1) $xBulanIndo="Jan";
	if($xBulanIndo == 2) $xBulanIndo="Feb";
	if($xBulanIndo == 3) $xBulanIndo="Mar";
	if($xBulanIndo == 4) $xBulanIndo="Apr";
	if($xBulanIndo == 5) $xBulanIndo="May";
	if($xBulanIndo == 6) $xBulanIndo="Jun";
	if($xBulanIndo == 7) $xBulanIndo="Jul";
	if($xBulanIndo == 8) $xBulanIndo="Aug";
	if($xBulanIndo == 9) $xBulanIndo="Sep";
	if($xBulanIndo == 10) $xBulanIndo="Oct";
	if($xBulanIndo == 11) $xBulanIndo="Nov";
	if($xBulanIndo == 12) $xBulanIndo="Dec";
	return $xBulanIndo;
}
...
Function tglIndo()
{
	...
	elseif($formatNya == "s_e2")
	{
		$TglIndo = $xTgl["mday"]." ".xBulanIndo3($xTgl["mon"])." ".($xTgl["year"] - $sisaTanggal);
	}
	elseif($formatNya == "t")
	{
...
/*function createThumbCut($gaFile,$gDir,$thumb_size,$gPath ) {
	...
	imagejpeg($new_im,$gPath.$gaFile,100);
 
}	*/

function getFirstImage($kalimat, $size, $style, $styleTag="") {
	...
	if(empty($imageDir)) {
		...
		if($style=="h"){
		$imageDir = '<img height="'.$size.'" '.$styleTag.' border="0" src="'.$imageDir.'"/>';
		}else $imageDir = '<img width="'.$size.'" '.$styleTag.' border="0" src="'.$imageDir.'"/>';
	}
...
function createThumbCut2($gaFile,$gDir,$thumb_size,$gPath ) {
	...
	$width = $size[0]; // lebar asli

	$height = $size[1]; // tinggi asli
	...
	$pembagix = $width / 200;
	$new_width = $width / $pembagix;
	$new_height = $height / $pembagix;
	
	$y = ceil(($new_height - 90) / 2 );
	if ($height > $width) {
		$y = ceil(($height - 90) / 2 );
	}
	if ($new_height < 90) {
		$pembagiy = $height / 90;
		$new_height = $height / $pembagiy;
		$new_width = $width / $pembagiy;
	$y = 0;
	
	$x = ceil(($new_width - 90) / 2 );
	}
	echo " ";
	echo "pembagi y = ".$pembagiy;
	echo " ";
	...
	$new_im = ImageCreatetruecolor(200,90);
