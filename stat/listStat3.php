...
if($cHidden=="1"){
	...
	if(!empty($katakunci)) {
		for($dd=0;$dd < count($arrKataKunci);$dd++){
			$detQuery=$detQuery." AND (nama LIKE '%".$arrKataKunci[$dd]."%' )";
	...
	$cmdTab = "show tables like 'stat_".$arr1[2]."_".$arr1[1]."'";
	//echo $cmdTab;			
	$result = mysql_query($cmdTab) or die ('error reading database');
	
	if (mysql_num_rows ($result)>0){					
						
		$sqlview = "SELECT * FROM stat_".$arr1[2]."_".$arr1[1]." 
				WHERE	id_radio = '".$_SESSION['SessionRadio']['radio']."'  
						". $detQuery ."  ORDER BY tgl DESC ";	
						
		$ada="ada";		
		$nm_table = "stat_".$arr1[2]."_".$arr1[1]."";
	...
	$cmdTab = "show tables like 'stat_".date("Y")."_".date("n")."'";
	//echo $cmdTab;			
	$result = mysql_query($cmdTab) or die ('error reading database');
	
	if (mysql_num_rows ($result)>0){
	
		$sqlview = "SELECT * FROM stat_".date("Y")."_".date("n")." WHERE id_radio = '".$_SESSION['SessionRadio']['radio']."' 
					 ".$detQuery."
						  ORDER BY tgl DESC ";	
						  
		$ada = "ada";				  
		$nm_table = "stat_".date("Y")."_".date("n")."";
	}
...
include "../inc/barHalaman_stat.php";
...
<link rel="stylesheet" type="text/css" media="all" href="../calendar_files/calendar-blue2.css" title="calendar-blue2" />
 ...
 <!-- main calendar program -->
  ...
  <script type="text/javascript" src="../calendar_files/calendar-setup.js"></script>
				...
				&nbsp;&nbsp;<span class="judul_menu">Daftar Statistik <?=$_SESSION['SessionRadio']['nama_radio'];?> ::</span>
	<br />
	<br />
	...
	<tr bgcolor="#BBBABA">
				   ...
				   <td valign=top align=center width="35%"><b>Referer</b></td>
				...
				$kReferer = $rs[referer];
				$kIp = $rs[ip];
				$kNama = $_SESSION['SessionRadio']['nama_radio'];
				...
				<tr bgcolor="#FFFFFF" onMouseOver="this.style.background='#ebebeb'" onMouseOut="this.style.background='#FFFFFF'">
					...
					<td valign=middle ><?echo $kReferer;?></td>
