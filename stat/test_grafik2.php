...
<tr><td colspan="3">Radio <b><?=$_SESSION['SessionRadio']['nama_radio'];?></b>
...
$p = 0;
	...
	$tmp30 = "AND		id_radio = '".$_SESSION['SessionRadio']['radio']."'";
	...
	if($_GET['waktu'] == 'bulan') {
		...
		for($k=1;$k<=date("t",mktime(0, 0, 0, $_GET['bulan'], 1,$_GET['tahun'] ));$k++) {
			...
			$cmdTab = "show tables like 'stat_".$_GET['tahun']."_".$_GET['bulan']."'";
			//echo $cmdTab;			
			$result = mysql_query($cmdTab) or die ('error reading database');
			
			if (mysql_num_rows ($result)>0){
	
				$cmd = "SELECT count(id_radio) As jml FROM stat_".$_GET['tahun']."_".$_GET['bulan']." 
						WHERE	tgl LIKE '".$_GET['tahun']."-".$bln."-".$tgl."%' ".$tmp30;
			
			
				//$cmd = "SELECT jumlah As jml FROM cni_summary WHERE bulan = '".$_GET['bulan']."' AND tahun = '".$_GET['tahun']."' ".$tmp30;	
		...
		for($k=1;$k<13;$k++) {
			...
			*/
			...
			$cmd = "SELECT jumlah As jml FROM cni_summary WHERE bulan = '".$k."' AND tahun = '".$_GET['tahun2']."' ".$tmp30;	
	...
	for($k=1;$k<=date("t",mktime(0, 0, 0, date("n"), 1,date("Y") ));$k++) {
		...
		if (mysql_num_rows ($result)>0){
			...
			$cmd = "SELECT count(id_radio) As jml FROM stat_".date("Y")."_".date("n")." 
