...
if($cHidden=="1"){
	...
	$cmdTab = "show tables like 'stat_durasi_".$arr1[2]."_".$arr1[1]."'";
	//echo $cmdTab;			
	$result = mysql_query($cmdTab) or die ('error reading database');
	
	if (mysql_num_rows ($result)>0){						
	
		$sqlview = "SELECT * FROM stat_durasi_".$arr1[2]."_".$arr1[1]."
				WHERE	 mount LIKE '/".$_SESSION['SessionRadio']['mount_radio']."%' 
						". $detQuery ."  ORDER BY tgl DESC ";		

		$ada="ada";	
		
		$nm_table = "stat_durasi_".$arr1[2]."_".$arr1[1]."";
	}						
	...
	if (mysql_num_rows ($result)>0){
		$sqlview = "SELECT * FROM stat_durasi_".date("Y")."_".date("n")." WHERE  mount LIKE '/".$_SESSION['SessionRadio']['mount_radio']."%'
					 ".$detQuery."
						  ORDER BY tgl DESC ";		
						  
		$ada = "ada";				  
		$nm_table = "stat_durasi_".date("Y")."_".date("n")."";
		//echo $sqlview;
 ...
 <!-- main calendar program -->
  ...
  <script type="text/javascript" src="../calendar_files/calendar-setup.js"></script>
	...
	<tr bgcolor="#BBBABA">
				...
				$hsl = explode('?',$_SESSION['SessionRadio']['mount_radio']);
				...
				<tr bgcolor="#FFFFFF" onMouseOver="this.style.background='#ebebeb'" onMouseOut="this.style.background='#FFFFFF'">
					...
					<td valign=middle ><?echo $_SESSION['SessionRadio']['nama_radio'];?></td>
