...
if($act=="hapus" && $id > 0){
	$sqlU="Update ".tabel_kota." set status='".$status."' where id_kota='".$id."'";
	//echo $sqlU;
	mysql_query($sqlU);
	}
...
if($cHidden=="1"){
	...
	for($dd=0;$dd < count($arrKataKunci);$dd++){
		$detQuery=$detQuery."(kota LIKE '%".$arrKataKunci[$dd]."%' )";
	
		
	}
	
	$sqlview = "SELECT * FROM ".tabel_kota." where ". $detQuery ."ORDER BY id_kota DESC";
	$sqlview =str_replace("AND ORDER","ORDER",$sqlview);
	
}else{
		$sqlview = "SELECT * FROM ".tabel_kota." ORDER BY id_kota DESC";
}
...
?>
				&nbsp;&nbsp;<span class="judul_menu">Daftar Kota ::</span>
	<br />
	<br />
	...
	<tr bgcolor="#BBBABA">
				   ...
				   <td valign=top align=center><b>Nama Kota</b></td>
				...
				<tr bgcolor="#FFFFFF" onMouseOver="this.style.background='#ebebeb'" onMouseOut="this.style.background='#FFFFFF'">
				   ...
				   <td valign=middle><a href="editKota.php?act=edit&id=<?echo $kID?>"><?echo $kKota;?></a></td>
				   
				   <td valign=top align=center>
				   <?
				   if($rs[status]=="0")
				   {
				   	echo '<a href="listKota.php?act=hapus&id='.$rs[id_kota].'&status=1&PageNo='.$PageNo.'"><img src="../images/on3.gif" alt="Active" border="0"></a>&nbsp;';
				   	echo '<img src="../images/off2.gif" border="0" alt="Set Inactive">';
				   }
				   else
				   {
				   	echo '<img src="../images/on2.gif" alt="Active">&nbsp;';
				   	echo '<a href="listKota.php?act=hapus&id='.$rs[id_kota].'&status=0&PageNo='.$PageNo.'"><img src="../images/off3.gif" border="0" alt="Set Inactive"></a>';
				   }
				   ?></td>
				</tr>
				<?
