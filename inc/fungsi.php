...
function Get_YMstatus($yahooid){
	$status = @file_get_contents("http://opi.yahoo.com/online?u=$yahooid&m=a&t=1");
    if ($status === '01')
     //return true;
     return '<img src="images/radio_ym1.png" border="0" width="18" height="19"/>';
	else
     //return false;
     return '<img src="images/radio_ym0.png" border="0" width="18" height="19"/>';
 }
...
function setStatistik($id=""){
	
	if(date("d") == "01"){
		
		$cmdCek = "SELECT * FROM cni_summary WHERE id_radio = '".$id."' ";
		$resCek = mysql_query($cmdCek);
		if(mysql_num_rows($resCek) > 0){
			$brsCek = mysql_fetch_array($resCek);
			if(date("n")-1 == 0){
				$bln = 12;
			}else{
				$bln = date("n")-1;
			...
			$cmdIns = "INSERT INTO cni_summary(id_radio,bulan,tahun,jumlah) 
						VALUES(
							'".$id."',
							'".$bln."',
							'".$tahun."',
							(SELECT COUNT(id_statistik) AS JML FROM stat_".$tahun."_".$bln." WHERE id_radio = '".$id."')
						)
					";
		...
		$cmdSt="CREATE TABLE `stat_".date("Y_n")."` (
				  `id_statistik` int(5) NOT NULL auto_increment,
				  `id_radio` int(5) NOT NULL default '0',
		...
		$cmdSt2 = "INSERT INTO `stat_".date("Y_n")."`(id_radio,tgl,referer,ip) 
			VALUES(
				'".$id."',
				now(),
		...
		$cmdSt="CREATE TABLE IF NOT EXISTS `stat_".date("Y_n")."` (
				  `id_statistik` int(5) NOT NULL auto_increment,
				  `id_radio` int(5) NOT NULL default '0',
				  `tgl` datetime NOT NULL default '0000-00-00 00:00:00',
				  `referer` varchar(255) NOT NULL default '',
...
function setDurasi($id="",$dur=0,$proses=""){
	
	if(date("d") == "01"){
		
		$cmdCek = "SELECT * FROM cni_summary_durasi WHERE mount = '".$id."' ";
		$resCek = mysql_query($cmdCek);
		if(mysql_num_rows($resCek) > 0){
			$brsCek = mysql_fetch_array($resCek);
			if(date("n")-1 == 0){
			...
			$cmdIns = "INSERT INTO cni_summary_durasi(mount,bulan,tahun,jumlah) 
						VALUES(
							'".$id."',
							'".$bln."',
							'".$tahun."',
							(SELECT SUM(duration) AS JML FROM stat_durasi_".$tahun."_".$bln." WHERE mount = '".$id."')
						)
					";
		...
		$cmdSt="CREATE TABLE `stat_durasi_".date("Y_n")."` (
				   `id` int(10) unsigned NOT NULL auto_increment,
				  `mount` varchar(255) NOT NULL default '',
				  `duration` varchar(50) NOT NULL default '',
				  `post_data` varchar(255) NOT NULL default '',
				  `tgl` datetime NOT NULL default '0000-00-00 00:00:00',
				  PRIMARY KEY  (`id`)
				) ;";
		...
		$cmdSt2 = "INSERT INTO `stat_durasi_".date("Y_n")."`(mount,duration,post_data,tgl) 
			VALUES(
				'".mysql_real_escape_string($id)."','".$dur."','".mysql_real_escape_string($proses)."',now()
			)";
		...
		$cmdSt="CREATE TABLE IF NOT EXISTS `stat_durasi_".date("Y_n")."` (
				   `id` int(10) unsigned NOT NULL auto_increment,
				  `mount` varchar(255) NOT NULL default '',
				  `duration` varchar(50) NOT NULL default '',
				  `post_data` varchar(255) NOT NULL default '',
...
function rankingHarian($idRadio)
	{
	//ranking harian mulai
	
				//cek tgl terakhir
				$cmd = "Select tgl from ranking_harian order by tgl desc limit 1";
				$result=mysql_query($cmd) or die();
				while($rs=mysql_fetch_array($result)){
					$tanggal_terakhir = $rs[tgl];
				}
				
				//jika tanggal dah lama maka jumlah di reset
				if($tanggal_terakhir!=date("Y-m-d"))
					{
					$cmd = "Update ranking_harian set jumlah='0', tgl=now()";
					mysql_query($cmd) or die();
					}
								
				//update statistik harian
				$cmd = "Update ranking_harian set jumlah=jumlah+1, ip='".$_SERVER['REMOTE_ADDR']."' where id_radio='".$idRadio."'";
				mysql_query($cmd) or die();
				if(mysql_affected_rows()==0) 
					{
					$cmd = "INSERT INTO ranking_harian(id_radio,tgl,jumlah,ip) 
						VALUES(
							'".$idRadio."',
							now(),
							'1',
	...
	//ranking harian selesai	
	}

function cekJumlahHit($id)
	{
		$jumlah= 0;
		if( mysql_num_rows( mysql_query("SHOW TABLES LIKE 'stat_".date("Y_n")."'")))
			{
				$interval = -5; //menit
				$tanggal=date("Y-m-d H:i:s",DateAdd("n",$interval,time()));
				$cmd = "Select count(*) as jumlah from`stat_".date("Y_n")."` where id_radio='".$id."' and ip='".$_SERVER['REMOTE_ADDR']."' and DATE_FORMAT(tgl,'%Y-%m-%d H:i:s') >= '".$tanggal."'";
				//echo $cmd;
...
function detail_radio($id){
	$row= _select_unique_result("SELECT r.*,tk.* FROM radio r
	left join t_kota tk on (tk.id_kota=r.id_kota)
	WHERE r.radio_id='$id' and r.status=1");
	return $row;
}
...
function getSocialMediaUI() {
	return '<div>
		    ...
		    <a class="addthis_button_compact addthis_32x32_style">share</a>
			</div>
