...
<div id="stationlist">
        ...
        <? $looping=mysql_query("Select * from radio where status=1");
		   $totalr=mysql_num_rows($looping);
		   $totalh=ceil($totalr/10);
		   $y=0;
		   for($x=1; $x<=$totalh; $x++){
		?>
  			<li>
  				 <ul class="garasi">
    				 <? $cmd = _select_arr("Select * from radio where status=1 order by radio_id limit $y,10");
