...
$selJenisUsahaUI = '';
$sqlJu = "select * from ".tabel_jenis_usaha." where status='1'";
$resJu = mysql_query($sqlJu);
while($rowJu=mysql_fetch_object($resJu)) {
	if ($katJu==$rowJu->id_ju) {
		$seld = 'selected="selected"';
		$nama_ju = $rowJu->jenis_usaha;
	} else {
		$seld = '';
	}
	$selJenisUsahaUI .= '<option '.$seld.' value="'.$rowJu->id_ju.'">'.$rowJu->jenis_usaha.'</option>';
}
$selJenisUsahaUI = '<select name="katJu"><option value="0">pilih jenis usaha</option>'.$selJenisUsahaUI.'</select>';
...
if($qact=="1")
{
  ...
  if(empty($phone)) $errmsg = $errmsg . "<li>Anda harus mengisi no telepon";
  if(!empty($phone) && !is_numeric($phone)) $errmsg = $errmsg . "<li>Format no telepon salah";
  if(empty($company)) $errmsg = $errmsg . "<li>Anda harus mengisi nama perusahaan Anda";
  if(empty($nama_ju)) $errmsg = $errmsg . "<li>Anda harus mengisi jenis usaha Anda";
  ...
  if(strlen($errmsg)<1)
  	{
		   ...
		   $include_pesan = $include_pesan . 	"Jenis Usaha: " . $nama_ju . "\r\n";
...
<form id="ajaxForm" method="post" action="<?echo $PHP_SELF;?>">
