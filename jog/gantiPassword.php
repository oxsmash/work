...
if($aksi=="1"){	
	...
	if(strlen($vPassLama) > 0) 
		{
		$queryCP = "SELECT * from ".tabel_user." where user_id='".$_SESSION['SessionNya']['id']."'";
		$resultCP=mysql_query($queryCP) or die("errorrrr");		
			while($row_arrayCP=mysql_fetch_array($resultCP)) {
			$vPassLama2=$row_arrayCP[user_pass];
			}
	...
	{
	$cmd = "UPDATE ".tabel_user." SET user_pass='".addslashes($crypt->Encrypt($vPass1,key_generator))."' where user_id='".$_SESSION['SessionNya']['id']."'";
