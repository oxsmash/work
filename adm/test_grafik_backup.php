...
if($_GET) {
	...
	if($_GET['waktu'] == 'bulan') {
		...
		for($k=1;$k<=date("t",mktime(0, 0, 0, $_GET['bulan'], 1,$_GET['tahun'] ));$k++) {
			...
			//$strTmp1000 = "listStat3.php?date1=".$tgl."-".$bln."-".$_GET['tahun']."&date2=".$tgl."-".$bln."-".$_GET['tahun']."&slRadio=".$_GET['slRadio']."&cHidden=1";
	...
	if($tot > 0) {
		...
		echo "<a href='".$strTmp1005."'>klik disini untuk lihat Log Radio</a>";
	...
	for($k=1;$k<=date("t",mktime(0, 0, 0, date("n"), 1,date("Y") ));$k++) {
		...
		//$strTmp1000 = "listStat3.php?date1=".$tgl."-".date("m")."-".date("Y")."&date2=".$tgl."-".date("m")."-".date("Y")."&slRadio=".$_GET['slRadio']."&cHidden=1";
