...
if($cHidden=="1"){
	...
	if(!empty($_GET['slRadio'])) {
		...
		$detQuery .= $detQuery . " AND mount LIKE '/".$_GET['slRadio']."%' ";
	...
	if (mysql_num_rows ($result)>0){
		$sqlview = "SELECT * FROM stat_durasi_".date("Y")."_".date("n")." WHERE mount != '' 
					 ".$detQuery."
						  ORDER BY tgl DESC ";		
  ...
  <script type="text/javascript" src="../calendar_files/calendar-setup.js"></script>
	...
	<tr bgcolor="#BBBABA">
				...
				<tr bgcolor="#FFFFFF" onMouseOver="this.style.background='#ebebeb'" onMouseOut="this.style.background='#FFFFFF'">
					...
					<td valign=middle ><?echo $arr[$hsl[0]];?></td>
